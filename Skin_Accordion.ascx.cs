...
{
    public partial class Skin_Accordion : BaseNavObjectBase
    {
        ...
        public string ViewMenuBind(List<TabInfo> tabs)
        {
            ...
            sb.AppendFormat("<ul  id=\"gomenu{0}\" class=\"gomenu {1}\">", ClientID,CssClass);
            ...
            for (int i = 0; i < tabs.Count; i++)
            {
                ...
                if (tab.ParentId == TopTabID && TopTabID != 0)
                {
                    ...
                    String aClass = String.Format("class=\"head {0} {1}\"", !String.IsNullOrEmpty(Str1) ? "parent" : "", !String.IsNullOrEmpty(SelectClass) ? "acurrent" : "");

                    sb.AppendFormat("<li class=\"{0} {1} {2} Item-{3} {4}\">", dirClass, ActiveClass, CircularBeadStyle, itemIndex, SelectClass);
                    sb.Append(this.FormatAlink(tab, aClass));
                    sb.Append(Str1);
                    sb.Append("</li>");
